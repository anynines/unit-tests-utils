  ...
  describe '#select' do
    ...
    context 'when no args are given' do
      it 'selects with default args' do
        expect(client).to receive(:execute).
          with("SELECT test_key, test_value FROM #{test_table} WHERE test_key = '#{test_key}'", {}).
          and_return(PG::Result.new()).once

        expect(client.select(test_key)).to_not be_nil
      end
    end
    ...
    context 'when args are given' do
      ...
      it 'passes the args to the client' do
        expect(client).to receive(:execute).
          with("SELECT test_key, test_value FROM #{test_table} WHERE test_key = '#{test_key}'", args).
          and_return(PG::Result.new()).once

        expect(client.select(test_key, args)).to_not be_nil
      end
    end
  end
