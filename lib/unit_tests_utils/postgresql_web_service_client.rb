...
class UnitTestsUtils::PGWebServiceClient
  ...
  DEFAULT_WEB_SERVICE_PASSWORD = 'password'.freeze
  ...
  def wait_cluster_is_ready(period = 5, retries = 1200)
    ...
    until is_cluster_ready
      ...
      raise StandardError, 'Timed out waiting for deployment to be ready' if counter >= retries
  ...
  def master_node
    ...
    raise StandardError, 'Could not reach any node'
  end

  def master_ip
    master_node['data']['node_ip']
  end
  ...
  def standby_nodes
    ...
    nodes.select { |node| !node.nil? && node['status'] == 'succeeded' && node['data']['mode'] == 'standby' }
  end
  ...
  def standby_min_age
    ...
    manifest.properties['postgresql-info-webservice']['monitor']['min_standby_age'].to_i
  end
  ...
  def check_interval
    ...
    manifest.properties['postgresql-info-webservice']['monitor']['heartbeat_interval'].to_i
  end
  ...
  def wait_monitor_block_node(node_id, sleeping_period = 5, retries = 1200)
    ...
    until is_blocked
      ...
      is_blocked = node['data']['blocked'] unless node.nil?

      sleep(sleeping_period)

      raise StandardError, 'Timed out waiting for node to be rejecting connections' if counter >= retries
  ...
  def wait_monitor_unblock_node(node_id, sleeping_period = 5, retries = 1200)
    ...
    while is_blocked
      ...
      raise StandardError, 'Timed out waiting for node to accept connections' if counter >= retries
  ...
  def cluster_replication_status
    nodes = node_ips.map do |node_ip|
      JSON.parse(
        HTTParty.get(
          "http://#{node_ip}:63145/v1/replication_status",
          basic_auth: webservice_credentials
        ).body
      )
    rescue Exception
      logger.debug("Could not reach node: #{node_ip}")
    ...
    nodes.select { |node| !node.nil? && node['status'] == 'success' }
  end
  ...
  def all_standby_nodes_unblocked(tolerance = 120)
    ...
    logger.debug(
      'Checking if whole cluster is accepting connections - ' \
    ...
    standby_nodes.each do |node|
      ...
      begin
        if pg_client.ping.positive?

          if node['data']['standby_age'] > min_age + tolerance
            raise StandardError,
                  'Standby node is rejecting connection after tolerance time - ' \
  ...
  def wait_standby_accept_connections(period = 5)
    ...
    sleep(period) until all_standby_nodes_unblocked
  ...
  def cluster_is_sync
    ...
    standby_ips.each do |node|
      ...
      return false if standby_lsn < master_lsn
    end
    true
  end
  ...
  def wait_cluster_is_sync(period = 5, retries = 1200)
    ...
    counter = 0
    until cluster_is_sync
      sleep(period)

      raise StandardError, 'Timed out waiting for cluster to replicate data' if counter >= retries
  ...
  def wait_failover(period = 5, retries = 1200)
    ...
    counter = 0
    until is_cluster_stable
      sleep(period)

      raise StandardError, 'Timed out waiting for cluster to complete failover' if counter >= retries
  ...
  def is_cluster_ready
    ...
    nodes = nodes_status.select do |node_status|
      node_status['status'] == 'succeeded' && node_status['data']['status'] == 'running'
    end

    nodes.length == nodes_status.length && has_a_master(nodes_status)
  end
